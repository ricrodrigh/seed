...
public:
	...
	byte getIndexOf( hash key ){
		for (byte i=0; i<size; i++){
			if(sizeof(hashMap[i].getHash()) == sizeof(key)) {
				String hashMapKey = hashMap[i].getHash();
		...
		/*
		Greates byte 255 will be returned if not found, implement hasKey() method, returning 0 
		if nothing is found is misleading. When building Map, ensure upper boundary is maxByteSize - 1
		*/
		return -1; 
	}
